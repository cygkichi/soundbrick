...
def marge_images(imagefiles, outfile, config):
    ...
    im = np.concatenate([im, np.zeros(num_ydots * 3 * np_x_add).reshape(
                        num_ydots, np_x_add, 3)], axis=1)

    num_ydots, num_xdots, _ = im.shape
    # print(np_x_add)
    # print(im.shape)
    im_stacked = np.concatenate(np.array([im.reshape(num_ydots, num_row,
                                num_xdots // num_row, 3)[:, i, :, :]
